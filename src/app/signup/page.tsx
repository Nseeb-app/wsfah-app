...
export default function SignupPage() {
  ...
  const signupWithPhone = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, phone, password: phonePassword }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);

      // Then sign in with OTP (server action)
      const result = await signupAndLogin("phone-otp", { phone, otp });
      if (result.success) {
        router.push("/");
        router.refresh();
      } else {
  ...
  const signupWithEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-login after registration (server action)
      const result = await signupAndLogin("credentials", { email, password: emailPassword });
      if (result.success) {
        router.push("/");
        router.refresh();
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1a0f0a] to-[#2D1B14] flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-sm">
        ...
        <button
          ...
          className="w-full py-3 rounded-xl bg-white text-slate-900 font-semibold text-sm flex items-center justify-center gap-3 hover:bg-slate-50 transition-colors"
        >
